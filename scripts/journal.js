    ...
    if (i + 1 != Object.keys(journals[journal]).length) {
      flipBook.innerHTML += `<div class="flip" id="p${i}" style="z-index: ${Object.keys(journals[journal]).length - i};">
        <div class="back">
          ...
          " alt="journal image"><label for="c${i}" class="next-button">Next &rarr;</label>` : `<img src="./images/logo.png" class="cover-image" alt="clue-logo"><h2>Journal</h2><p class="journal-team">${journal} Team</p><label for="c${i}" class="next-button">Begin &rarr;</label>`}
        </div>
        <div class="front">
          <h2>${entry[0]}</h2>
          ${entry[1]}
